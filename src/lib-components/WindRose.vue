...
import { toNumber, toString, toUpper } from "../lib/util";

require("../lib/RGraph.common.core");
require("../lib/RGraph.rose");
  ...
  methods: {
    init() {
      ...
      if (!this.buffers.background) {
        ...
        this.drawCompassPoints(this.buffers.ctxBackground, size);
      }
      ...
      this.buffers.plot.style.display = "none";
      this.buffers.plot.id = "rosePlot";
      this.buffers.ctxPlot = this.buffers.plot.getContext("2d");
      this.$refs["div"].appendChild(this.buffers.plot);

      // Create a new rose plot (16 points)
      //const data = [10, 5, 8, 3, 9, 4, 6, 10, 5, 3, 6, 8, 4, 9, 15, 12];
      const data = [0, 0, 0, 0, 0, 0, 0, 10, 0, 0, 0, 0, 0, 0, 0, 0];
      const rose = new RGraph.Rose({
        id: "rosePlot",
        data: data,
        options: {
          margin: Math.ceil(40 / data.length),
          colors: ["Gradient(#408040:red:#7070A0)"],
          colorsAlpha: 0.5,
          colorsStroke: "black",
          axes: true,
          axesColor: "grey",
          backgroundGridRadialsCount: data.length,
        },
      });
      rose.draw();
    ...
    drawCompassPoints(ctx, size) {
      ...
      for (var i = 0; i < 4; i++) {
        ...
        ctx.fillText(compassPoints[i * 2], 0, 0, size);
        ctx.translate(-size / 2, -size * 0.125);
        // Move to center
